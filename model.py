...
class Initializer(nn.Module):
    def __init__(self, drug_dim, cline_dim, out_dim, use_disease=False, use_GMP=False):
        ...
        self.drug_conv_same = nn.ModuleList([gnn.Seqential('x, edge_index',[
    ...
    def forward(self, drug_x, drug_adj, ibatch, cline_x, *args): 
        ...
        if self.use_disease:
            ...
            for i in range(self.disease_same_layers):
                disease_x = disease_x +self.disease_samep[i](disease_x)
...
class Refiner(torch.nn.Module):
        def __init__(self, in_dim, out_dim):
            ...
            self.conv_same = nn.ModuleList([nn.Sequential('x, hyperedge_index, hyperedge_weight',[
                (nn.BatchNorm1d(hidden_dim), 'x -> x'),
                (gnn.HypergraphConv(hidden_dim, hidden_dim), 'x, hyperedge_index, hyperedge_weight -> x'),
                (nn.ReLU(), 'x -> x'),
                
            ])]for _ in range(self.hdn_layers))

            init_bias = 0.0
            self.conv_w_out = nn.ModuleList([nn.Seqential(
...
class HERMERS(torch.nn.Module):
    def __init__(self, bio_encoder, graph_encoder, decoder, use_disease=False):
        ...
        self.use_disease = use_disease
        if self.use_disease:
            sefl.initializer.set_use_disease()
    ...
    def forward(self, drug_x, drug_adj, ibatch, cline_x, druga_id, drugb_id, cline_id, *args):
        ...
        noise_scale = 0.0
        if not self.traning and noise_scale :
            drug_x += torch.rand_like(drug_x) * noise_scale
            cline_x += torch.rand_like(cline_x) * noise_scale
            if self.use_disease:
                disease_features = args[0]
                disease_features += torch.rand_like(
                    disease_features) * noise_scale
            
        if self.use_disease:
            disease_features = args[0]
            drug_embed, cline_embed, disease_embd = self.initializer(
                drug_x, drug_adj, ibatch, cline_x, disease_features)
        ...
        rec_cline = torch.sigmoid(torch.einsum(
            'i d, d e, j e -> i j', cline_emb, self.drug_rec_weight, cline_emb)
        )

        if self.initializer.use_disease:
            disease_emb = graph_embed[self.num_drug +
                                      self.num_cline:]
        ...
        preds = self.consolidator(
            graph_embed, druga_id, drugb_id, cline_id
        )
        return preds, rec_s